...
@test "Create python3 base image" {
    ...
    run dispatch create base-image python3-base ${image_url} --language python3
    echo_to_log
    assert_success

    run_with_retry "dispatch get base-image python3-base --json | jq -r .status" "READY" 8 5
}

@test "Create python3 image" {
    run dispatch create image python3 python3-base
    echo_to_log
    assert_success
